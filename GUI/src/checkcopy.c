...
//copying all word to heap memmory
	...
	for(i=0,j=0;i<linecnt;i++){
		while(wordS1=strtok_r(ptrtxt1[i],dictOCT,&ptrtxt1[i])){
			cntWord1++;
	...
	for(i=0,j=0;i<linecnt;i++){
		while(wordS2=strtok_r(ptrtxt2[i],dictOCT,&ptrtxt2[i])){
			cntWord2++;
	...
	for(i=0;i<cntWord1;i++){
		//printf("%d %s",i,wordCpy1[i]); //debug_line_can_be_removed
		strtok(wordCpy1[i],"\n");
	}
	...
	for(i=0;i<cntWord1;i++){
		for(j=0;j<cntWord2;j++){
			...
			if(strcmp(wordCpy1[i],wordCpy2[j])==0){
				countC++;
				//printf("%d %d <%s><%s>%d\n",i,j,wordCpy1[i],wordCpy2[j],countC); //debug_line_can_be_removed
				break;
			}
		}
	}
